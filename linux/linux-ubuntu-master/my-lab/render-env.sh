...
# env.properties -> 환경 변수 로딩 + ./*.t -> *.yaml 생성

ENV_FILE="env.properties"
TEMPLATE_DIR="." # 현재 디렉토리(env.properties 이하) 기준
...
# TEMPLATE_DIR="." 기준으로 현재 디렉터리 이하 전체를 재귀
# find의 출력 결과를 while의 표준 입력(stdin) 으로 넘김 == find가 뱉은 걸 read가 한 줄씩 읽기
done < <(find "$TEMPLATE_DIR" -type f -name "*.t" -print0)
